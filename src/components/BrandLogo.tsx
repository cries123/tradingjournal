...
const fullHeights = {
  sm: 'h-20 max-w-[140px]',
  md: 'h-24 max-w-[168px]',
  lg: 'h-24 sm:h-28 md:h-32 max-w-[220px] md:max-w-none',
};

const compactText = {
  sm: { title: 'text-[10px]', sub: 'text-[8px]', gap: 'gap-2' },
  md: { title: 'text-[11px]', sub: 'text-[9px]', gap: 'gap-2.5' },
  lg: { title: 'text-xs', sub: 'text-[10px]', gap: 'gap-3' },
};
  ...
  return (
    <div className={`flex items-center ${text.gap} min-w-0 max-w-full`}>
      ...
      <div className="min-w-0 leading-none">
        ...
        <p className={`${text.sub} text-[#8e939d] mt-0.5 hidden sm:block`}>Track · Analyze · Improve</p>
      </div>
    </div>
  );
}
