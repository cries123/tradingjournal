export interface ChangelogEntry {
  /**
   * "2.3" — the release this shipped in, and the thing a reader scans for.
   *
   * Numbered in ship order rather than derived from anything: there is no build that carries a
   * version, so this is the changelog's own numbering and the changelog is the only place it is
   * ever written down. The one rule that gives it meaning is the major digit — 2.0 is the release
   * the product started charging on, so a 1.x entry is the free-journal era and a 2.x entry is
   * not. Bump the major only when the answer to "what is this product" changes; everything else
   * is the next minor.
   */
  version: string;
  ...
  date: string;
  title: string;
  /** Blank-line separated. The page renders each `\n\n` as its own paragraph. */
  description: string;
}

// Add new entries to the TOP of this array as real updates ship — newest first, with the next
// version number up from the one above it.
//
